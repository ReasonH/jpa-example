...
import com.example.transactionaltestintegration.service.async.UpdateAsyncOuterService;
...
public class UpdateAsyncOuterServiceTest {
    ...
    @Test
    @Order(3)
    @DisplayName("스레드가 영속성 객체를 수정하는 함수를 호출, 영속성 컨텍스트가 전파되지 않으며, 내부 dirty-checking이 이루어지지 않는다.")
    ...
    @Test
    @Order(6)
    @DisplayName("Async 함수에서 업데이트 수행, caller가 일정 시간 대기 후 종료되면서 dirty-checking이 동작한다.")
